...
func (maker *PasetoMaker) GenerateToken(email string, gender int, userID string, duration time.Duration) (string, *Payload, error) {
	payload, err := NewPayload(email, gender, userID, duration)
	if err != nil {
		return "", payload, err
	}
